...
가게 카드를 펼치면 나오는 "리뷰 N개 분석 결과"는 `VITE_N8N_REVIEW_WEBHOOK_URL`이 설정되어 있으면 **네이버 블로그
검색으로 실제 후기**를 가져옵니다 (n8n 웹훅 `matjib-reviews`). 가져온 글 본문에 `src/utils/reviewClassifier.ts`의
문구 패턴(예: "협찬", "체험단", "제공받아", "원고료")을 적용해 광고성 여부를 분류합니다. 이 패턴은 실제
표시광고법상 고지 문구를 참고해 만들었습니다. 블로그 검색 결과에는 별점이 없어 리뷰 카드에 별점은 표시되지
않고, "원문 보기" 링크로 실제 글로 이동할 수 있습니다.

웹훅 URL이 비어 있으면(로컬 개발 등) 대신 `src/utils/generateReviews.ts`가 데모용 가짜 리뷰를 생성해 같은 UI로
보여줍니다 — 실제 데이터가 아니라는 점에 유의하세요.
