...
def main() -> int:
    ...
    표, 바뀐횟수 = 날마다고르기_지표(
        지표, 앞, 거래일, 처음키,
        학습창=인자.학습창, 앞선날=인자.앞선날, 이웃=인자.이웃,
        최소학습=인자.최소학습,
    )
    쓴전략 = sorted(set(표.values()))
    print(f"■ 지표로 고르기: {바뀐횟수}번 바꿨고 전략 {len(쓴전략)}개를 거쳤다",
          file=sys.stderr)

    낸것: dict[str, dict] = {}

    print("■ 지표로 고른 계좌를 굴린다", file=sys.stderr)
    ...
    print(f"■ 무작위 대조군 {인자.무작위횟수}번", file=sys.stderr)
    무작위들 = []
    for 씨 in range(인자.무작위횟수):
        ...
        ㅁ요약 = 요약(ㅁ해별, 인자.평가시작해)
        무작위들.append(ㅁ요약)
        print(f"  씨 {씨:2} 평균 {ㅁ요약['평균']} 최악 {ㅁ요약['최악']}", file=sys.stderr)
    if 무작위들:
        ...
        낸것["무작위 대조군"] = {
            ...
            "지표가 이긴 비율": round(
                float(np.mean([낸것["지표로 고르기"]["요약"]["최악"] > m for m in 최악들])), 3
            ) if 최악들 and 낸것["지표로 고르기"]["요약"]["최악"] is not None else None,
        }
    ...
    print()
    print("=" * 72)
    print(f"지표로 고르기   평균 {ㅈ['요약']['평균']}%  최악 {ㅈ['요약']['최악']}% "
          f"({ㅈ['요약'].get('최악해')}년)  낙폭 {ㅈ['전체낙폭']}%  거래 {ㅈ['거래']}건")
    if "무작위 대조군" in 낸것:
        ㅁ = 낸것["무작위 대조군"]
        print(f"무작위 대조군   최악의 중앙값 {ㅁ['최악의 중앙값']}%  "
              f"가장 나은 것 {ㅁ['최악의 최고']}%  지표가 이긴 비율 {ㅁ['지표가 이긴 비율']}")
    for 키 in 기준전략:
        if 키 in 고정:
            ㄱ = 고정[키]
            print(f"{ㄱ['이름']:14} 평균 {ㄱ['요약']['평균']}%  최악 {ㄱ['요약']['최악']}% "
                  f"({ㄱ['요약'].get('최악해')}년)  낙폭 {ㄱ['전체낙폭']}%  거래 {ㄱ['거래']}건")
    print("=" * 72)
    return 0
