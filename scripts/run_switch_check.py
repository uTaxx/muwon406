...
def main() -> int:
    ...
    print(f"\n  {'규칙':30} {'수익률':>8} {'사고판것':>7} {'끝에보유':>7} "
          f"{'승률':>6} {'최대낙폭':>9} {'변경':>5}")
    print("  " + "-" * 80)
    for ㅈ in 잰것:
        print(f"  {ㅈ['이름']:30} {ㅈ['수익률']:+7.2f}% {ㅈ['거래수']:6}건 "
              f"{ㅈ['보유수']:6}개 {ㅈ['승률']:5.1f}% {ㅈ['최대낙폭']:+8.2f}% "
              f"{ㅈ['변경횟수']:4}회")
    print("\n  사고판것은 **사고 판 것까지 끝난 매매**만 셉니다. 아직 들고 있는 "
          "종목은 끝에보유에 있습니다.")
    print("  구간이 짧으면 한 바퀴가 안 끝나서, 수익률의 대부분이 아직 안 판 "
          "종목의 그날 값입니다.")
    print()
    for ㅈ in 잰것:
