...
def 기준이름() -> list[dict]:
    ...
    표 |= _디비칸
    # 시트 초안에 넣는 값. 화면이 n8n 연결에서 진짜 값을 못 받았을 때
    # 이 값으로 칸을 채운다. 전에는 화면에 예시 숫자를 손으로 적어 두었는데
    # (비중 20%, 동시보유 5종목) 실제 기본값(15%, 8종목)과 달랐다. 예시라고
    # 적혀 있어도 숫자는 그럴듯해 보여서, 지금 걸린 값으로 읽힌다.
    기본값 = {b.이름: b.기본 for b in 기준들}
    return [
...
def 기간설명() -> list[dict]:
    ...
    고치고 둘을 잊는다."""
    return [
        {"이름": ㄱ.이름, "달수": ㄱ.달수, "나눔": ㄱ.나눔, "설명": ㄱ.설명}
        for ㄱ in 기간들
    ]
