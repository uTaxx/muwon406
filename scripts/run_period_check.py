...
def 전략고르기(글: str) -> tuple[list[str], list[str], bool]:
    ...
    **모르는 열쇠말은 조용히 버리지 않고 여기서 멈춘다.** 하나를 잘못 적었는데
    ...
    # 파는 쪽을 견줄 때는 사는 쪽이 하나여야 한다. 둘 다 돌리면 표의 한 줄이
    # 무엇을 바꾼 결과인지 알 수 없다.
    파는쪽견주기 = 파는쪽.strip() in ("전부", "all")
    if 파는쪽견주기:
        사는키 = [ㄱ.strip() for ㄱ in 사는쪽.split(",") if ㄱ.strip()]
        if len(사는키) != 1 or 사는키[0] in ("전부", "all"):
            raise ValueError(
                "파는 쪽을 견줄 때는 사는 쪽을 전략 하나로 적으세요. "
...
def 비교하기(골라진것, 잰때: datetime, sheet_id: str, histories, 끝, 정책,
    ...
    **여기서 나온 1등을 그대로 걸면 안 된다.** 한 구간에서 제일 좋았던 것을
    ...
    `파는쪽견주기`면 사는 쪽을 고정하고 **파는 쪽을 갈아 끼우며** 잰다.
    "사는 조건이 나쁜 건가 파는 조건이 나쁜 건가"를 가르는 자리다. 그때
    표의 한 줄은 매수 전략이 아니라 매도 전략을 가리킨다."""
    파는키들 = list(파는키들 or [])
    고정매수 = 열쇠들[0] if 파는쪽견주기 else ""
    파는글 = "" if 파는쪽견주기 else ",".join(파는키들)
    if 파는쪽견주기:
        print(f"■ 매수 전략을 {전략이름(고정매수)} 하나로 고정합니다")
        print(f"■ 견주는 매도 전략 {len(파는키들)}개\n")
    else:
    ...
    def 만들기(열쇠: str):
        ...
        파는 쪽을 견줄 때는 열쇠가 매도 전략이고 매수는 고정이다. 아니면
        열쇠가 매수 전략이다.
    ...
    # 파는 쪽을 견줄 때는 줄이 매도 전략이다.
    도는것 = 파는키들 if 파는쪽견주기 else 열쇠들
    for 정의 in 골라진것:
        줄들 = []
        못만든것 = []
        for 열쇠 in 도는것:
