...
def 전략고르기(글: str) -> tuple[list[str], list[str], bool]:
    """`--전략` 값을 (사는키들, 파는키들, 파는쪽전부비교)로 푼다.

    받는 모양은 다섯이다.

        지금                          지금 DB에 걸린 것 (이때는 여기까지 안 온다)
        전부                          등록된 전략 전부를 사는 쪽으로 비교한다
        volume_surge_3d               그 전략 하나
        volume_surge_3d>macd_cross    사는 쪽과 파는 쪽을 따로
        volume_surge_3d>전부          **사는 쪽을 고정하고 파는 쪽을 비교한다**

    사는 쪽에 여럿을 적으려면 쉼표로 잇는다. `>` 뒤가 파는 쪽이고, 안 적으면
    사는 쪽이 파는 것까지 맡는다. `>` 뒤에 쉼표로 여럿을 적으면 그것들을
    OR로 묶어 **하나의** 파는 규칙을 만든다. 하나씩 갈아 끼워 비교하는 것은
    `>전부`뿐이다. 둘은 다른 물음이라 같은 글자로 쓰면 안 된다.
    ...
    **모르는 열쇠말은 조용히 버리지 않고 여기서 멈춘다.** 하나를 잘못 적었는데
    ...
    # 파는 쪽을 비교할 때는 사는 쪽이 하나여야 한다. 둘 다 돌리면 표의 한 줄이
    # 무엇을 바꾼 결과인지 알 수 없다.
    파는쪽전부비교 = 파는쪽.strip() in ("전부", "all")
    if 파는쪽전부비교:
        사는키 = [ㄱ.strip() for ㄱ in 사는쪽.split(",") if ㄱ.strip()]
        if len(사는키) != 1 or 사는키[0] in ("전부", "all"):
            raise ValueError(
                "파는 쪽을 비교할 때는 사는 쪽을 전략 하나로 적으세요. "
                "둘 다 돌리면 표의 한 줄이 무엇을 바꾼 결과인지 알 수 "
                f"없습니다: {글!r}"
            )
        모르는것 = [ㄱ for ㄱ in 사는키 if ㄱ not in set(아는열쇠())]
        if 모르는것:
            raise ValueError(
                "등록되지 않은 전략입니다: " + ", ".join(모르는것)
                + " · 쓸 수 있는 것: " + ", ".join(sorted(아는열쇠()))
            )
        return 사는키, 아는열쇠(), True


    def 나누기(조각: str) -> list[str]:
        return [ㄱ.strip() for ㄱ in 조각.split(",") if ㄱ.strip()]

    사는키 = 아는열쇠() if 사는쪽.strip() in ("전부", "all") else 나누기(사는쪽)
    파는키 = 나누기(파는쪽)
    if not 사는키:
...
def 전략글(사는키: str, 파는키: str) -> str:
    ...
    **파는 쪽을 안 걸었을 때 이름만 적으면 안 된다.** 그러면 파는 규칙이
    없는 것으로도, 살 때 조건 그대로 파는 것으로도 읽힌다. 실제로는 전략
    하나가 사는 조건과 파는 조건을 따로 들고 있다(2026-08-29에 물어보심)."""
    산것 = 전략이름(사는키)
    if 파는키:
        return f"{산것}\n매도 전략은 따로 걸었습니다. {전략이름(파는키)}입니다."
    return f"{산것}\n매도 전략은 따로 안 걸어서 이 전략의 파는 규칙대로 팝니다."
...
def 화면에(성적들: list, 사는키: str, 파는키: str, 기준: str,
    ...
    print(
        ...
        if 파는키 else "■ 매도 전략: 따로 안 걸었습니다 (매수 전략의 파는 규칙대로)"
    )
    print(f"■ 기준: {기준}\n")
    for 성적 in 성적들:
        ㅁ = 성적.metrics
...
def main() -> int:
    ...
    받은것.add_argument(
        "--나눔", default="",
        help="구간 안을 무엇으로 나눌 것인가 (날/주/달/해). 안 적으면 구간마다 정한 대로",
    )
    받은것.add_argument(
        "--유니버스", default="시가총액",
    ...
    # 나누는 단위를 손으로 바꾼다. "3개월이 나쁜데 최근에도 계속 나쁜가"는
    # 달로 나누면 토막이 셋뿐이라 안 보인다. 주로 나누면 열세 토막이 된다.
    #
    # **시트에는 안 올린다.** 쌓이는 줄은 구간마다 정한 나눔으로 잰 것이라야
    # 지난 회차와 이어진다. 여기서 바꾼 것을 같이 쌓으면 같은 칸에 뜻이
    # 다른 값이 섞인다.
    나눌것 = 인자.나눔.strip()
    if 나눌것:
        if 나눌것 not in _조사:
            raise SystemExit(f"모르는 나눔입니다: {나눌것} (쓸 수 있는 것: {list(_조사)})")
        골라진것 = [replace(ㄱ, 나눔=나눌것) for ㄱ in 골라진것]
    ...
    sheet_id = "" if (인자.no_sheet or 나눌것) else 시트찾기(인자)
    if 나눌것:
        print(f"■ 나눔을 {나눌것}로 바꿔서 봅니다. 시트에는 안 올립니다.")
...
def 대상종목(인자, sheet_id: str, session_factory):
    ...
    **종류를 기준 글에 적는다.** 시트에는 두 종류로 잰 줄이 같이 쌓이므로,
    어느 목록으로 잰 것인지 줄마다 남아 있어야 나중에 비교할 수 있다."""
...
def 비교하기(골라진것, 잰때: datetime, sheet_id: str, histories, 끝, 정책,
    ...
    **여기서 나온 1등을 그대로 걸면 안 된다.** 한 구간에서 제일 좋았던 것을
    ...
    `파는쪽전부비교`면 사는 쪽을 고정하고 **파는 쪽을 갈아 끼우며** 잰다.
    "사는 조건이 나쁜 건가 파는 조건이 나쁜 건가"를 가르는 자리다. 그때
    표의 한 줄은 매수 전략이 아니라 매도 전략을 가리킨다."""
    파는키들 = list(파는키들 or [])
    고정매수 = 열쇠들[0] if 파는쪽전부비교 else ""
    파는글 = "" if 파는쪽전부비교 else ",".join(파는키들)
    if 파는쪽전부비교:
        print(f"■ 매수 전략을 {전략이름(고정매수)} 하나로 고정합니다")
        print(f"■ 비교하는 매도 전략 {len(파는키들)}개\n")
    else:
        print(f"■ 비교하는 매수 전략 {len(열쇠들)}개")
        print("■ 매도 전략: "
              + (", ".join(전략이름(ㄱ) for ㄱ in 파는키들) if 파는키들
                 else "따로 안 걸었습니다 (각 전략의 파는 규칙대로)")
              + "\n")
    올릴것: list[list[str]] = []
    ...
    def 만들기(열쇠: str):
        """표의 한 줄에 해당하는 전략을 만든다.

        파는 쪽을 비교할 때는 열쇠가 매도 전략이고 매수는 고정이다. 아니면
        열쇠가 매수 전략이다.

        파는 쪽을 안 적었으면 감싸지 않는다. 감싸면 기록에 남는 전략 이름이
        바뀌어서 지금까지 쌓인 것과 이어지지 않는다."""
        if 파는쪽전부비교:
            return build_strategies([고정매수], "OR", [열쇠])
        if not 파는키들:
            return build_strategy(열쇠)
        return build_strategies([열쇠], "OR", 파는키들)

    # 파는 쪽을 비교할 때는 줄이 매도 전략이다.
    도는것 = 파는키들 if 파는쪽전부비교 else 열쇠들
    for 정의 in 골라진것:
        줄들 = []
        못만든것 = []
        for 열쇠 in 도는것:
        ...
        올릴것.extend(
            비교줄만들기(잰때, 고정매수 or ㅋ, ㅅ, 기준,
                    ㅋ if 파는쪽전부비교 else 파는글, 총평)
            for ㅋ, ㅅ in 줄들
        )
        비교표(정의, 줄들, 파는글)
        print(f"  {총평}\n")
    ...
    if not 올릴것:
        raise RuntimeError("비교할 결과가 한 줄도 안 나왔습니다.")
