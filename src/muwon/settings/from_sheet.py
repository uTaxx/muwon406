...
기준들: tuple[기준, ...] = (
    ...
    기준(
        ...
        "완전 자동이 무서우면 이걸 켠다. 켠 채로 며칠 실행해 보고 제안이 "
        "납득되면 그때 끄는 것이 순서다",
    ),
    기준(
    ...
    기준(
        ...
        "이만큼 오르면 판다 (0이면 전략이 정한 대로)",
        "**0은 전략에게 맡긴다는 뜻이다**(2026-09-02에 뜻이 바뀌었다). "
        "전에는 0이 곧 '끔'이었는데, 전략마다 익절선을 다르게 두려면 그 "
        "자리가 필요했다. 지금 등록된 전략 27개 중 익절선을 정한 것이 "
        "하나도 없어서 0은 여전히 '안 건다'와 같은 결과다. "
        "여기에 숫자를 넣으면 전략이 정한 값을 덮는다",
        정책필드="take_profit_pct", 최소=0, 최대=100,
    ),
    기준(
        "max_holding_days", "보유 기간 상한(일)", "정수", "0",
        "며칠까지 들고 있을지. 0이면 전략이 정한 대로 (지금 전략은 5일)",
        "**0은 '안 정했다'가 아니라 '전략에게 맡긴다'는 뜻이다.** 지금까지의 "
        "전략 평가 결과가 전부 그 상태에서 나온 숫자다. 여기에 숫자를 넣으면 전략이 "
        "정한 기간을 덮어쓰고, 그 순간부터 전략 평가 결과는 지금 동작을 설명하지 않는다",
        정책필드="max_holding_days", 최소=0, 최대=60,
    ),
    기준(
        "min_turnover_eok", "거래대금 최소 기준(억)", "숫자", "50",
    ...
    기준(
        "rank_1st", "1순위 판단 지침", "글자", "worst_slice",
        "전략을 견줄 때 제일 먼저 보는 것",
        "**이 칸은 주문을 내지 않는다. 무엇을 먼저 볼지만 정한다.** 매일 저녁 "
        "검토와 기간 검증의 순위가 이 순서로 매겨진다. 2026-09-03까지는 "
        "순위 코드가 수익률로만 줄을 세웠고, 저장소가 적어 둔 1순위 기준(가장 "
        "나빴던 구간)과 어긋나 있었다. 고를 수 있는 값은 "
        "`analysis/judgment.py`의 순위 지침 목록에 있다",
        텔레그램가능=False,
    ),
    기준(
        "rank_2nd", "2순위 판단 지침", "글자", "max_drawdown",
        "1순위가 비슷할 때 다음으로 보는 것",
        "지침마다 '이 정도 차이는 같다고 본다'는 폭이 있고, 그 폭 안이면 "
        "이 지침으로 가른다. 폭이 없으면 0.01%p 차이가 순위를 가르고 "
        "2순위와 3순위는 영영 안 쓰인다",
        텔레그램가능=False,
    ),
    기준(
        "rank_3rd", "3순위 판단 지침", "글자", "total_return",
        "1순위와 2순위가 다 비슷할 때 마지막으로 보는 것",
        "셋은 서로 달라야 한다. 같은 것을 두 자리에 넣으면 뒤 자리가 아무 "
        "일도 안 한다. 성하지 않으면 기본값으로 돌아가고, 그 사실을 "
        "검토 로그에 적는다",
        텔레그램가능=False,
    ),
    기준(
