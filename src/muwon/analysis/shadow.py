...
## 어떻게 재나
    ...
    제안일          지평이 지난 날
      │                  │
      ├── 후보 전략 ─────┤   이 구간을 후보 전략으로 돌렸으면 몇 %
      └── 그때 전략 ─────┘   같은 구간을 그때 걸려 있던 전략으로 돌리면 몇 %
                             둘의 차이가 이 표가 남기는 값이다

같은 종목, 같은 기간, 같은 기초설정으로 돌린다. 다른 것은 전략 하나뿐이다.
...
class 비교:
    ...
    지난날수: int
    등급: str
    #: 그날 실제로 버튼이 나갔나. 안 나간 날도 순위 1위로 견준다.
    버튼있었나: bool
...
def 기록하기(
    ...
    **같은 날 두 번 돌리면 앞의 것을 지우고 다시 쓴다.** 두 벌이 남으면
    나중에 견줄 때 같은 날이 두 번 세어진다."""
    ...
    for 구간이름, 순위 in 순위들.items():
        제안키 = 제안키들.get(구간이름, "")
        # 순위 위쪽 몇 개에 더해, 그때 걸려 있던 전략과 버튼으로 나간 후보는
        # 순위가 아래여도 반드시 남긴다. 견주는 짝이 둘 다 있어야 한다.
...
def 견주기(줄들) -> list[비교]:
    ...
    버튼이 안 나간 날도 순위 1위를 후보 자리에 놓고 견준다. 그날 후보를
