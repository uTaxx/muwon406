...
전략 성적표(`report_card.py`)가 **이미 잰 것**을 담는다면, 이 파일은
**아직 못 잰 것**을 담는다. 실시간(장중) 매매는 지금 성적이 하나도 없는
상태라, 화면에 성적표를 띄우면 빈 표가 나오고 "고장인가"로 읽힌다.
...
`docs/실시간계획.json`에 적어 두고 여기서 읽는다. 성적표와 같은 이유로
...
GRADES = {
    ...
    "C": ("orange", "독립 재현: 제3자가 코드로 다시 돌려 봤다"),
