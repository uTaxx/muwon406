...
# ── 제목 줄 늘리기 ─────────────────────────────────────────────────
#
# 탭은 처음 만들 때만 제목을 적는다. 나중에 칸이 늘면 값은 오른쪽에 붙는데
# 제목은 그대로라, 시트를 열어 본 사람이 그 칸이 뭔지 알 수가 없다.

def test_칸이_늘면_제목_줄을_늘린다():
    from muwon.cloud.sheet_log import 머리늘려야하나

    assert 머리늘려야하나(["열쇠", "때"], ["열쇠", "때", "평가"]) is True


def test_앞쪽이_다르면_제목을_안_건드린다():
    """사람이 제목을 고쳐 뒀을 수도 있다. 그것을 코드가 덮으면 안 된다."""
    from muwon.cloud.sheet_log import 머리늘려야하나

    assert 머리늘려야하나(["열쇠", "시각"], ["열쇠", "때", "평가"]) is False


def test_칸이_안_늘었으면_아무것도_안_한다():
    from muwon.cloud.sheet_log import 머리늘려야하나

    assert 머리늘려야하나(["열쇠", "때", "평가"], ["열쇠", "때", "평가"]) is False
    assert 머리늘려야하나(["열쇠", "때", "평가", "덤"], ["열쇠", "때", "평가"]) is False
