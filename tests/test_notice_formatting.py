"""텔레그램에 안 먹는 기호가 알림에 다시 들어오는 것을 막는다.

`TelegramNotifier.send`는 `parse_mode`를 안 준다. 그래서 `**굵게**`가
강조가 아니라 별 네 개 그대로 도착한다. `★`도 마찬가지로, 무슨 뜻인지
아무도 모른다.
...
이 파일은 문체를 검사하지 않는다. 여기 있는 것은 전부 렌더링 문제이거나
내부 이름이 새는 문제다. 2026-08-27까지는 줄표(—) 금지 검사도 여기 있었다.
그건 취향이지 결함이 아니어서 뺐다. 받아 보는 사람이 "문장이 하나도 이해가
안 된다"고 했고, 원인을 따라가 보니 이런 규칙들이 문장을 토막 내고 있었다.
문장을 어떻게 쓸지는 CLAUDE.md에서도 규칙으로 두지 않는다.
