...
def test_기간이_짧은_것부터_다섯이다():
    ...
    # 나누는 단위는 구간 길이를 따라간다. 5년을 달로 나누면 토막이 예순 개라
    # 제일 나빴던 한 달은 언제나 크게 나쁘고 그래서 아무 말도 못 한다.
    # 반대로 1개월을 달로 나누면 토막이 하나라 구간 전체와 같아진다.
    assert 기간표["1주"].나눔 == "날"
    assert 기간표["1개월"].나눔 == "주"
    assert 기간표["3개월"].나눔 == "달"
    assert 기간표["5년"].나눔 == "해"
...
def test_날과_주로_나눈_토막에_이름이_붙는다():
    """이름이 시간 순으로 정렬돼야 제일 나빴던 토막을 옳게 찾는다."""
    from muwon.analysis.period_check import 구간수익률
...
def test_곡선이_짧으면_토막이_없다():
    assert 구간수익률(_곡선([(date(2026, 1, 1), 100.0)]), "달") == []
    assert 구간수익률(None, "달") == []
...
def _성적(이름="3개월", **덮개):
    ...
    return 기간성적(
        ...
        구간들=덮개.get("구간들", [("2026-06", -7.0), ("2026-07", 1.0)]),
        모자람=덮개.get("모자람", ""),
    )
...
def test_해로_나눈_구간이_다_플러스면_그렇게_적는다():
    from muwon.analysis.period_check import 평가글

    글 = 평가글(
        _성적("5년", 구간들=[("2021년", 6.7), ("2022년", 12.0)]), 기간표["5년"]
    )
...
def test_파는쪽만_갈아_끼워_비교할_수_있다():
    ...
    사는키, 파는키, 비교하기 = 전략고르기("volume_surge_5d_ma20>전부")
    assert 사는키 == ["volume_surge_5d_ma20"]
    assert 파는키 == 아는열쇠()
    assert 비교하기 is True

    # 쉼표는 예전 뜻 그대로다. 둘을 같은 글자로 쓰면 안 된다.
    _, 파는키, 비교하기 = 전략고르기("volume_surge_5d_ma20>macd_cross,ma_rsi_v1")
