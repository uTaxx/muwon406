...
> 리스크 관리 미션의 핵심 참조 문서. 계열사별로 확인된 공개 리스크 요인만 기록하며,
> 확인되지 않은 리스크는 추정하지 않는다.

## 1. 리스크 카테고리 정의

- 소송·제품책임 (Litigation / Product Liability)
...
- 카테고리: 소송·제품책임, 산업안전, 정책·규제
- 현재 확인된 공개 리스크 사실: TODO: source required (본 문서는 Pilot 최초 구축 시점 기준으로
  비어 있으며, WF-P05 실행 후 INTELLIGENCE_DB의 확정 사실이 누적되면 이 섹션에 요약을 반영한다)
...
이 문서는 사람이 수기로 채우는 정적 문서가 아니라, WF-P05 심층분석 결과 중 `significance`가
높은 건이 누적되면 주기적으로 요약을 반영하는 문서다 (자동 갱신은 Pilot 범위 밖이며, TASK-012
Dashboard가 이 역할의 상당 부분을 대체한다). 이 파일은 Claude Context에 로드되는 "확정된
배경지식" 용도로 유지한다.
