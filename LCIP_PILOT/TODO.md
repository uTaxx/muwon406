...
## Round 13 이어서: 대시보드 상호작용 범위 재확인 (2026-08-08)

사용자가 "직접 눈으로 보고 수정하고 실행할 수 있는" 대시보드를 기대했으나, 현재
Dashboard는 정적 HTML(볼 수만 있음)이고 "실행" 카드는 클릭형 버튼이 아니라 CLI
명령어 텍스트임을 확인시켜줬다(Round 11에서 "백엔드 서버 없는 정적 HTML+CLI
구조"로 명시적으로 결정된 사항). 원래 계획인 TASK-016(Natural Language Admin,
자연어로 설정 변경 요청→승인→적용)은 Round 4부터 계속 보류 상태였다는 점도
재확인했다. 사용자에게 3가지 선택지(CLI 유지 / 버튼형 실행 백엔드 추가 / TASK-016
