...
(() => {
  ...
  const 시계 = [
    ["08:40", "시장·섹터 리포트", "어제 장이 어땠는지 텔레그램으로"],
    ["08:50", "매수 후보 제안", "오늘 살 만한 것을 골라 알림 — 여기서 승인하세요"],
    ...
    if (!다음) {
      $("다음실행").insertAdjacentHTML("beforeend",
        `<div class="곁말" style="margin-top:8px">오늘 예정된 것은 다 끝났습니다 —
         다음은 ${평일 ? "내일" : "돌아오는 평일"} 08:40입니다.</div>`);
    }
  }
  ...
  const 알림예시 = { 알림: [
    ...
    { 때: "08-24 08:50", 종류: "제안", 글: "오늘 매수 후보 3종목 — 화면에서 골라 주세요." },
  ] };
  ...
  function 기준그리기(자료, 진짜) {
    ...
    ["킬전환", "손절", "비중", "동시보유", "기준저장"].forEach((id) => {
      $(id).disabled = !진짜;
    });
  }
  ...
  $("저장").addEventListener("click", () => {
    ...
    보이기("본문", true);
    불러온적.clear();
    (용어들.length ? Promise.resolve() : 자료읽기()).then(() => 탭보이기("손익"));
  });
